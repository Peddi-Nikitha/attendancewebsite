...
} from "@/lib/firebase/hooks/useEmployeeDocuments";

export default function AdminEmployeeDocumentsClient() {
  const searchParams = useSearchParams();
  const employeeId = useMemo(() => searchParams.get("id") || "", [searchParams]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader title={employeeId || "Select an employee"} subtitle="Employee ID" />
        <CardContent>
          <div className="mb-3">
            <Button type="button" onClick={() => setOpen(true)} disabled={!employeeId}>
              Add Document
            </Button>
          </div>
          {!employeeId ? (
              ...
              {docs.map((d) => (
                ...
                >
                  ...
                  <Button
                    ...
                    disabled={deleting}
                    onClick={async () => {
                      await deleteDoc(d.id, d.filePath);
                      await refresh();
                    }}
                  >
                    Delete
            ...
            <form onSubmit={onSubmit} className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-500">
                  Leave empty to use document type + filename
                </p>
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-slate-600">
              ...
              <div className="flex items-center justify-end gap-2">
                ...
                <Button type="submit" disabled={uploading || !employeeId}>
